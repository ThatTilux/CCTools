...
int main() {
    ...
    CCTools::Logger::info("Now creating a ModelHandler object...");
    CCTools::ModelHandler model_handler("../examples/cct.json");

    // Get the harmonic drive values for the B components
    CCTools::Logger::info("Now getting the harmonic drive values for the B components...");
    CCTools::HarmonicDriveParameterMap param_map_B = model_handler.getHarmonicDriveValues("B");

    // Print the drive values
    CCTools::Logger::info("Printing the harmonic drive values for the B components...");
    ...
    for (auto [name, param] : param_map_B) {
        ...
        CCTools::Logger::info(name + ": " + to_string(param));
    }

    // Override the B1 drive values. B1 has an amplitude of constant, so we need to set a corresponding constant value
    CCTools::Logger::info("Now setting the harmonic drive values for B1...");
    // Set it to 5e-05
    CCTools::HarmonicDriveParameters params_B1(5e-05, CCTools::HarmonicDriveParameterType::Constant);
    // Option 1:
    model_handler.setHarmonicDriveValue("B1", params_B1);
    // Option 2:
    CCTools::HarmonicDriveParameterMap new_param_map_B;
    ...
    CCTools::Logger::info("Now getting the updated harmonic drive values for the B components...");
    ...
    CCTools::Logger::info("Now creating a ModelCalculator object...");
    ...
    CCTools::Logger::info("Now running the harmonics calculation...");
    ...
    CCTools::Logger::info("Printing the b_n values...");
    // b_n values are stored in a vector
    std::vector<double> bn_values = harmonics_handler.get_bn();
    for (size_t i = 0; i < bn_values.size(); i++) {
        CCTools::Logger::info("b_" + std::to_string(i + 1) + ": " + std::to_string(bn_values[i]));
    }

    // Change the B1 value again to see the results change
    CCTools::Logger::info("Now setting the harmonic drive values for B1 again...");
    // Set it to 1e-04
    CCTools::HarmonicDriveParameters params_B1_new(1e-04, CCTools::HarmonicDriveParameterType::Constant);
    model_handler.setHarmonicDriveValue("B1", params_B1_new);

    // Run the harmonics calculation again
    CCTools::Logger::info("Now running the harmonics calculation again...");
    ...
    CCTools::Logger::info("Printing the updated b_n values...");
